...
public interface XocDiaBetRepository extends JpaRepository<XocDiaBet, Long> {
    ...
    @Query("SELECT b FROM XocDiaBet b WHERE b.settledAt IS NOT NULL AND (:status IS NULL OR b.status = :status) AND (:start IS NULL OR COALESCE(b.settledAt, b.createdAt) >= :start) AND (:end IS NULL OR COALESCE(b.settledAt, b.createdAt) <= :end) ORDER BY b.settledAt DESC")
    Page<XocDiaBet> findForAnalytics(@Param("status") XocDiaBet.Status status,
                                     @Param("start") Instant start,
                                     @Param("end") Instant end,
                                     Pageable pageable);

    @Query("SELECT COALESCE(SUM(b.stake), 0) FROM XocDiaBet b WHERE (:status IS NULL OR b.status = :status) AND (:start IS NULL OR COALESCE(b.settledAt, b.createdAt) >= :start) AND (:end IS NULL OR COALESCE(b.settledAt, b.createdAt) <= :end)")
    BigDecimal sumStakeByFilters(@Param("status") XocDiaBet.Status status,
                                 @Param("start") Instant start,
                                 @Param("end") Instant end);

    @Query("SELECT COALESCE(SUM(b.winAmount), 0) FROM XocDiaBet b WHERE (:status IS NULL OR b.status = :status) AND (:start IS NULL OR COALESCE(b.settledAt, b.createdAt) >= :start) AND (:end IS NULL OR COALESCE(b.settledAt, b.createdAt) <= :end)")
    BigDecimal sumWinAmountByFilters(@Param("status") XocDiaBet.Status status,
                                     @Param("start") Instant start,
                                     @Param("end") Instant end);

    @Query("SELECT COALESCE(SUM(b.stake), 0) FROM XocDiaBet b WHERE b.status IN :statuses AND (:start IS NULL OR COALESCE(b.settledAt, b.createdAt) >= :start) AND (:end IS NULL OR COALESCE(b.settledAt, b.createdAt) <= :end)")
    BigDecimal sumStakeByStatusesAndDate(@Param("statuses") Collection<XocDiaBet.Status> statuses,
                                         @Param("start") Instant start,
                                         @Param("end") Instant end);

    @Query("SELECT COALESCE(SUM(b.feeAmount), 0) FROM XocDiaBet b WHERE b.status = com.xsecret.entity.XocDiaBet$Status.WON AND (:start IS NULL OR COALESCE(b.settledAt, b.createdAt) >= :start) AND (:end IS NULL OR COALESCE(b.settledAt, b.createdAt) <= :end) AND b.feeAmount IS NOT NULL")
    ...
    List<Object[]> aggregateTotalsByUsers(
            ...
            @Param("start") Instant start,
            @Param("end") Instant end,
            @Param("refundedStatus") XocDiaBet.Status refundedStatus,
            @Param("lostStatus") XocDiaBet.Status lostStatus
    );
    ...
    @Query("""
        SELECT COALESCE(SUM(b.winAmount - b.stake), 0) FROM XocDiaBet b
        WHERE b.status = com.xsecret.entity.XocDiaBet$Status.WON
          AND (:start IS NULL OR b.createdAt >= :start)
          AND (:end IS NULL OR b.createdAt <= :end)
    """)
    ...
    @Query("""
        SELECT COALESCE(SUM(b.winAmount), 0) FROM XocDiaBet b
        WHERE b.user = :user 
          AND b.status = com.xsecret.entity.XocDiaBet$Status.WON
          AND (:start IS NULL OR b.createdAt >= :start)
          AND (:end IS NULL OR b.createdAt <= :end)
    """)
    ...
    @Query("""
        SELECT COALESCE(SUM(CASE WHEN b.status <> com.xsecret.entity.XocDiaBet$Status.REFUNDED THEN b.stake ELSE 0 END), 0)
        FROM XocDiaBet b
        WHERE b.user = :user
    """)
    ...
    @Query("""
        SELECT COALESCE(SUM(b.stake), 0) FROM XocDiaBet b
        WHERE b.user = :user 
          AND b.status = com.xsecret.entity.XocDiaBet$Status.LOST
          AND (:start IS NULL OR b.createdAt >= :start)
          AND (:end IS NULL OR b.createdAt <= :end)
    """)
